...
const VendorLogin = ({ isEmbedded = false, onSwitchToRegister }) => {
  ...
  const handleSendOTP = async (e) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/vendors/login-send-otp`, {
        method: 'POST',
  ...
  const handleVerifyAndLogin = async (e) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/vendors/login-verify-otp`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok && data.success) {
        if (data.isRegistered === false) {
          ...
          navigate('/auth?tab=vendor', { state: { mobile: phoneNumber, otp: otpValue, redirectToRegister: true } });
        } else {
    ...
    <div className={containerClasses}>
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm mx-auto">
        ...
        <AnimatePresence mode="wait">
          <motion.div key="vendor-otp-login-form" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
            <div className="mb-8">
              ...
              <p className="text-[14px] font-bold text-neutral-500 tracking-tight leading-snug">
                {isOtpSent 
                  ...
                  : "Enter your registered mobile number to receive login OTP"}
              </p>
            </div>
                ...
                <div className="space-y-4 pt-2">
                  ...
                  <div className="flex justify-between gap-3">
                    {otp.map((digit, i) => (
                      <input 
                        key={i} 
                        id={`vendor-login-otp-${i}`} 
                        type="text" 
                        maxLength={1} 
                        value={digit}
                        onChange={(e) => handleOtpChange(i, e.target.value)}
                        onKeyUp={(e) => e.key === 'Backspace' && i > 0 && !digit && document.getElementById(`vendor-login-otp-${i-1}`).focus()}
